...
type sessionWindowMockRepo struct {
	...
	rateLimitCalls     []sessionWindowRateLimitCall
}
...
type sessionWindowRateLimitCall struct {
	ID      int64
	ResetAt time.Time
}
...
func (m *sessionWindowMockRepo) SetRateLimited(_ context.Context, id int64, resetAt time.Time) error {
	m.rateLimitCalls = append(m.rateLimitCalls, sessionWindowRateLimitCall{ID: id, ResetAt: resetAt})
	return nil
}
