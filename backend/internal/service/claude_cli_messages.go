...
func prependClaudeCLILongToolDescriptionReminder(message *claudeCLISDKMessage, descriptions []claudeCLILongToolDescription) {
	...
	b.WriteString("Some client tool descriptions are too long for Claude Code's MCP tool description limit, so their full descriptions are provided here. Use these full descriptions for the corresponding MCP tools.\n\n")
...
func isClaudeCodeGeneratedMessageText(text string) bool {
	trimmed := strings.TrimSpace(text)
	normalized := strings.ToLower(strings.Join(strings.Fields(trimmed), " "))
	return strings.HasPrefix(strings.ToLower(trimmed), "<system-reminder>") &&
		strings.Contains(normalized, "as you answer the user's questions") &&
		strings.Contains(normalized, "# currentdate")
}
