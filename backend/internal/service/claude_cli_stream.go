...
func writeClaudeCLIMessageJSONAsAnthropicSSE(w io.Writer, data []byte) error {
	var message struct {
		ID           string           `json:"id"`
		Type         string           `json:"type"`
		Role         string           `json:"role"`
	...
	if err := json.Unmarshal(data, &message); err != nil {
		return fmt.Errorf("claude cli stream: decode translated message for sse: %w", err)
	}

	envelope := &claudeCLIFinalAssistantEnvelope{Type: "assistant"}
	envelope.Message.ID = message.ID
	envelope.Message.Type = message.Type
	envelope.Message.Role = message.Role
	envelope.Message.Model = message.Model
	envelope.Message.Content = message.Content
	envelope.Message.StopReason = message.StopReason
	envelope.Message.StopSequence = message.StopSequence
	envelope.Message.Usage = message.Usage
	return writeFinalAssistantAsAnthropicSSE(w, envelope, claudeCLITranslateOptions{})
}
