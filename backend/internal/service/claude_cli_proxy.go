...
func (p *ClaudeCLIProxy) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest, startTime time.Time) (*ForwardResult, error) {
	...
	if len(input.Tools) > 0 {
		...
		toolGroups := splitClaudeCLIToolsByMCPServer(input.Tools)
		configServers, session, cleanup, err := mcpServer.Register(toolGroups, claudeCLIPendingToolCallTTL(account))
		if err != nil {
			return nil, err
		}
		mcpSession = session
		cleanupMCP = cleanup
		logClaudeCLIMCPDebug("proxy registered mcp session=%s server_count=%d tool_count=%d", session.ID, len(configServers), len(input.Tools))
		mcpConfig, err = buildClaudeCLIMCPConfig(configServers)
		if err != nil {
			cleanupMCP()
			return nil, err
		}
	}
	configuredClaudeCLIUserID := account.GetClaudeCLIUserID()
	ws, err := prepareClaudeCLIWorkspace(p.tempDir, input, claudeCLIWorkspaceOptions{
		SessionID: sessionID,
		Now:       now,
		MCPConfig: mcpConfig,
		UserID:    configuredClaudeCLIUserID,
	})
	if err != nil {
		if cleanupMCP != nil {
			cleanupMCP()
	...
	if isClaudeCLIDebugEnabled() {
		logClaudeCLIDebug("prepared workspace dir=%s session_path=%s system_prompt_path=%s mcp_config_path=%s args=%v history_count=%d tool_count=%d configured_user_id=%t", ws.Dir, ws.SessionPath, ws.SystemPromptPath, ws.MCPConfigPath, ws.Args, len(input.History), len(input.Tools), configuredClaudeCLIUserID != "")
		logClaudeCLIDebugPayload("input.system_prompt", input.SystemPrompt)
		logClaudeCLIDebugPayload("input.history", input.History)
		logClaudeCLIDebugPayload("input.prompt", input.Prompt)
		logClaudeCLIDebugPayload("workspace.stdin", ws.Stdin)
		logClaudeCLIDebugPayload("workspace.mcp_config", mcpConfig)
	}
	...
	if len(input.Tools) > 0 {
		return p.forwardWithClientTools(ctx, c, account, parsed, startTime, input, auth, ws, stdin, &stderr, runner, mcpSession.ID, cleanupMCP, configuredClaudeCLIUserID)
	}
	defer ws.Cleanup()
	if cleanupMCP != nil {
		defer cleanupMCP()
	...
	if input.Stream {
		...
		runErr := <-errCh
		claudeCLIUserID := claudeCLIWorkspaceUserIDForResult(ws, configuredClaudeCLIUserID)
		logClaudeCLIDebugPayload("process.stdout", stdoutPreview.String())
		logClaudeCLIDebugPayload("process.stderr", stderr.String())
		logClaudeCLIDebug("process.completed stream=true err=%v", runErr)
		if translateErr != nil {
			if stderr.Len() > 0 {
	...
	if err := runner.Run(ctx, claudeCLIProcessRequest{
		...
		claudeCLIUserID := claudeCLIWorkspaceUserIDForResult(ws, configuredClaudeCLIUserID)
		logClaudeCLIDebugPayload("process.stdout", stdout.String())
		logClaudeCLIDebugPayload("process.stderr", stderr.String())
		logClaudeCLIDebug("process.completed stream=false err=%v", err)
		if stdout.Len() > 0 {
	...
	logClaudeCLIDebugPayload("process.stdout", stdout.String())
	logClaudeCLIDebugPayload("process.stderr", stderr.String())
	logClaudeCLIDebug("process.completed stream=false err=<nil>")
...
func (p *ClaudeCLIProxy) forwardWithClientTools(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest, startTime time.Time, input *claudeCLIInput, auth claudeCLIProcessAuth, ws *claudeCLIWorkspace, stdin io.Reader, stderr *bytes.Buffer, runner claudeCLIProxyRunner, mcpSessionID string, cleanupMCP func(), configuredClaudeCLIUserID string) (*ForwardResult, error) {
	...
	select {
	case mcpErr := <-stdoutCollector.MCPFailure():
	...
	case call := <-toolCallCh:
		...
		logClaudeCLIDebugPayload("proxy.tool_calls", calls)
	...
	case runErr := <-errCh:
		output := <-stdoutDone
		logClaudeCLIDebugPayload("process.stdout", output.data)
		logClaudeCLIDebugPayload("process.stderr", stderr.String())
		logClaudeCLIDebug("process.completed client_tools=true err=%v", runErr)
		p.closePendingToolRun(pending, false)
		if output.err != nil {
			return nil, fmt.Errorf("claude cli proxy: read stdout: %w", output.err)
		}
		if mcpErr := claudeCLIMCPFailureFromOutput(output.data); mcpErr != nil {
			return nil, mcpErr
		}
		...
		if runErr != nil {
			if len(filtered) > 0 {
				...
				if result != nil {
					result.ClaudeCLIUserID = claudeCLIWorkspaceUserIDForResult(ws, configuredClaudeCLIUserID)
				}
				return result, writeErr
			}
...
func writeClaudeCLIToolUseResponse(c *gin.Context, input *claudeCLIInput, parsed *ParsedRequest, calls []claudeCLIToolCall, prefixBlocks []map[string]any) error {
	...
	if parsed != nil && parsed.Stream {
		...
		for _, call := range calls {
			clientToolName := claudeCLIClientToolName(call.Name)
			logClaudeCLIDebug("response.tool_use stream=true call_id=%s cli_name=%s client_name=%s", call.ID, call.Name, clientToolName)
			logClaudeCLIDebugPayload("response.tool_use.input", call.Input)
			events = append(events,
				mustClaudeCLIRawJSON(map[string]any{"type": "content_block_start", "index": visibleIndex, "content_block": map[string]any{"type": "tool_use", "id": call.ID, "name": clientToolName, "input": map[string]any{}}}),
	...
	for _, call := range calls {
		clientToolName := claudeCLIClientToolName(call.Name)
		logClaudeCLIDebug("response.tool_use stream=false call_id=%s cli_name=%s client_name=%s", call.ID, call.Name, clientToolName)
		logClaudeCLIDebugPayload("response.tool_use.input", call.Input)
...
func claudeCLIClientToolName(name string) string {
