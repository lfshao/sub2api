...
func TestClaudeCLIProxyForwardRegistersToolsByMCPServer(t *testing.T) {
	gin.SetMode(gin.TestMode)

	bridgeTools := make(map[string][]claudeCLITool)
	runner := &fakeClaudeCLIProcessRunner{
		run: func(req claudeCLIProcessRequest) error {
			bridgeTools["claude"] = readMCPHTTPToolsFromRequest(t, req, "claude")
			bridgeTools["jetbrains"] = readMCPHTTPToolsFromRequest(t, req, "jetbrains")
			_, err := io.WriteString(req.Stdout, claudeCLIStreamJSON(
				`{"type":"message_start","message":{"id":"msg_1","role":"assistant","model":"claude-opus-4-7","usage":{"input_tokens":1}}}`,
				`{"type":"message_delta","delta":{"stop_reason":"end_turn"},"usage":{"output_tokens":1}}`,
				`{"type":"message_stop"}`,
	...
	body := []byte(`{"model":"claude-opus-4-7","messages":[{"role":"user","content":"hi"}],"tools":[{"name":"lookup","description":"Lookup a thing","input_schema":{"type":"object"}},{"name":"mcp__jetbrains__read_file","description":"Read a file","input_schema":{"type":"object"}},{"name":"WebFetch","description":"Fetch a URL","input_schema":{"type":"object"}}]}`)
	parsed, err := ParseGatewayRequest(body, PlatformAnthropic)
	require.NoError(t, err)
	account := &Account{
		Platform:    PlatformAnthropic,
	...
	requireArgFollowedBy(t, runner.req.Args, "--tools", "")
	requireClaudeCLIToolsByName(t, bridgeTools["claude"], map[string]claudeCLITool{
	...
	requireClaudeCLIToolsByName(t, bridgeTools["jetbrains"], map[string]claudeCLITool{
		"read_file": {Name: "read_file", Description: "Read a file", InputSchema: map[string]any{"type": "object"}},
	})
}

func TestClaudeCLIProxyForwardWaitsForExistingMCPServerToolCalls(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	runner := &fakeClaudeCLIProcessRunner{
		runWithContext: func(ctx context.Context, req claudeCLIProcessRequest) error {
			...
			_, err := io.WriteString(req.Stdout, claudeCLIStreamJSON(
				...
				`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","id":"toolu_jetbrains","name":"mcp__jetbrains__get_repositories","input":{}}}`,
			...
			go func() {
				resultCh <- callMCPToolFromRequestWithBodyForServer(
					t,
					req,
					"jetbrains",
					[]byte(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"get_repositories","arguments":{},"_meta":{"claudecode/toolUseId":"toolu_jetbrains"}}}`),
					toolCallStarted,
				)
			}()
			select {
	...
	body := []byte(`{"model":"claude-opus-4-7","messages":[{"role":"user","content":"idea?"}],"tools":[{"name":"mcp__jetbrains__get_repositories","description":"Get repositories","input_schema":{"type":"object"}}]}`)
	parsed, err := ParseGatewayRequest(body, PlatformAnthropic)
	require.NoError(t, err)
	account := &Account{
		Platform:    PlatformAnthropic,
	...
	defer cancel()
	result, err := proxy.Forward(ctx, c, account, parsed, time.Now())
	require.NoError(t, err)
	require.NotNil(t, result)
	waitForChannel(t, toolCallStarted, "jetbrains mcp tool call")
	require.Contains(t, rec.Body.String(), `"name":"mcp__jetbrains__get_repositories"`)
	require.Contains(t, rec.Body.String(), `"id":"toolu_jetbrains"`)

	var message map[string]any
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &message))
	content := requireSliceAny(t, message["content"])
	toolUseID := requireStringAny(t, requireMapAny(t, content[0])["id"])
...
func TestClaudeCLIProxyForwardRegistersWebToolsInMCPBridgeWithoutAccountEnabledTools(t *testing.T) {
	...
	runner := &fakeClaudeCLIProcessRunner{
		run: func(req claudeCLIProcessRequest) error {
			bridgeTools = readMCPHTTPToolsFromRequest(t, req, "claude")
			_, err := io.WriteString(req.Stdout, claudeCLIStreamJSON(
				`{"type":"message_start","message":{"id":"msg_1","role":"assistant","model":"claude-opus-4-7","usage":{"input_tokens":1}}}`,
				`{"type":"message_delta","delta":{"stop_reason":"end_turn"},"usage":{"output_tokens":1}}`,
				`{"type":"message_stop"}`,
...
func TestClaudeCLIProxyForwardWithToolsFailsWhenMCPServerFailed(t *testing.T) {
	gin.SetMode(gin.TestMode)

	runCtxDone := make(chan struct{})
	runner := &fakeClaudeCLIProcessRunner{
		runWithContext: func(ctx context.Context, req claudeCLIProcessRequest) error {
			_, err := io.WriteString(req.Stdout, `{"type":"system","subtype":"init","tools":["Monitor"],"mcp_servers":[{"name":"tools","status":"failed"}],"model":"claude-opus-4-7"}`+"\n")
			require.NoError(t, err)
			select {
			case <-ctx.Done():
				close(runCtxDone)
	...
	body := []byte(`{"model":"claude-opus-4-7","messages":[{"role":"user","content":"hi"}],"tools":[{"name":"Bash","description":"Run shell commands","input_schema":{"type":"object"}}]}`)
	parsed, err := ParseGatewayRequest(body, PlatformAnthropic)
	require.NoError(t, err)
	account := &Account{
		Platform:    PlatformAnthropic,
	...
	_, err = proxy.Forward(context.Background(), c, account, parsed, time.Now())
	require.ErrorContains(t, err, `mcp server "tools" failed to connect`)
	select {
	case <-runCtxDone:
	case <-time.After(time.Second):
		t.Fatal("claude cli process was not cancelled after MCP connection failure")
	}
...
func TestClaudeCLIProxyForwardCompletesClientToolCallAcrossRequests(t *testing.T) {
	...
	thinking, ok := firstContent[0].(map[string]any)
	require.True(t, ok)
	...
	firstToolUse, ok := firstContent[1].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "lookup", firstToolUse["name"])
	toolUseID, ok := firstToolUse["id"].(string)
	require.True(t, ok)
	...
	secondContent, ok := secondMessage["content"].([]any)
	require.True(t, ok)
	text, ok := secondContent[0].(map[string]any)
	require.True(t, ok)
...
func TestWriteClaudeCLIToolUseResponseStripsMCPServerPrefix(t *testing.T) {
	...
	content := requireSliceAny(t, message["content"])
	require.Len(t, content, 1)
	toolUse := requireMapAny(t, content[0])
...
func TestClaudeCLIProxyForwardCompletesMultipleClientToolCallsAcrossRequests(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal(firstRec.Body.Bytes(), &firstMessage))
	require.Equal(t, "tool_use", firstMessage["stop_reason"])
	firstContent := requireSliceAny(t, firstMessage["content"])
	require.Len(t, firstContent, 2)
	toolUseByName := make(map[string]map[string]any)
	for _, rawBlock := range firstContent {
		block := requireMapAny(t, rawBlock)
		toolUseByName[requireStringAny(t, block["name"])] = block
	}
	require.Contains(t, toolUseByName, "lookup")
	require.Contains(t, toolUseByName, "fetch")
	firstToolUseID := requireStringAny(t, toolUseByName["lookup"]["id"])
	secondToolUseID := requireStringAny(t, toolUseByName["fetch"]["id"])
	...
	require.NoError(t, json.Unmarshal(secondRec.Body.Bytes(), &secondMessage))
	require.Equal(t, "end_turn", secondMessage["stop_reason"])
	text := requireMapAny(t, requireSliceAny(t, secondMessage["content"])[0])
...
func TestClaudeCLIProxyForwardCompletesClientToolCallInSameCLIProcess(t *testing.T) {
	...
	var firstMessage map[string]any
	require.NoError(t, json.Unmarshal(firstRec.Body.Bytes(), &firstMessage))
	firstToolUse := requireMapAny(t, requireSliceAny(t, firstMessage["content"])[0])
	firstToolUseID := requireStringAny(t, firstToolUse["id"])
	...
	require.NoError(t, json.Unmarshal(secondRec.Body.Bytes(), &secondMessage))
	require.Equal(t, "end_turn", secondMessage["stop_reason"])
	content := requireSliceAny(t, secondMessage["content"])
	require.Len(t, content, 1)
	text := requireMapAny(t, content[0])
...
func TestClaudeCLIProxyForwardReturnsNextToolCallAfterToolResult(t *testing.T) {
	...
	var secondMessage map[string]any
	require.NoError(t, json.Unmarshal(secondRec.Body.Bytes(), &secondMessage))
	secondToolUse := requireMapAny(t, requireSliceAny(t, secondMessage["content"])[0])
	require.Equal(t, "fetch", secondToolUse["name"])
	secondToolUseID := requireStringAny(t, secondToolUse["id"])
	...
	text := requireMapAny(t, requireSliceAny(t, thirdMessage["content"])[0])
...
func TestClaudeCLIProxyForwardWritesLatestToolResultToSessionAndContinuesWithContinuePrompt(t *testing.T) {
	...
	userMessage := requireMapAny(t, userLine["message"])
	userContent := requireSliceAny(t, userMessage["content"])
	toolResult := requireMapAny(t, userContent[0])
	require.Equal(t, "tool_result", toolResult["type"])
	require.Equal(t, "toolu_expired", toolResult["tool_use_id"])

	var response map[string]any
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &response))
	content := requireSliceAny(t, response["content"])
	text := requireMapAny(t, content[0])
...
func readMCPHTTPToolsFromRequest(t *testing.T, req claudeCLIProcessRequest, serverName string) []claudeCLITool {
	t.Helper()
...
func callMCPToolFromRequestWithBody(t *testing.T, req claudeCLIProcessRequest, payload []byte, started chan<- struct{}) string {
	t.Helper()
	return callMCPToolFromRequestWithBodyForServer(t, req, "claude", payload, started)
}

func callMCPToolFromRequestWithBodyForServer(t *testing.T, req claudeCLIProcessRequest, serverName string, payload []byte, started chan<- struct{}) string {
	t.Helper()
	mcpConfigPath := requireArgFollowedBy(t, req.Args, "--mcp-config", "")
	rawConfig, err := os.ReadFile(mcpConfigPath)
	require.NoError(t, err)
...
func requireMapAny(t *testing.T, value any) map[string]any {
	t.Helper()
...
func requireSliceAny(t *testing.T, value any) []any {
	t.Helper()
	out, ok := value.([]any)
	require.True(t, ok)
	return out
...
func requireStringAny(t *testing.T, value any) string {
	t.Helper()
	out, ok := value.(string)
	require.True(t, ok)
	return out
